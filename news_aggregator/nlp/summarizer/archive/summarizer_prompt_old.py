...
def create_prompt(content, article_length, include_images=True, enable_entity_links=True):
    """
    Create a detailed prompt for summarizing article content with advanced formatting and multimodal capabilities.
    ...
    - include_images (bool): Whether to search for and include relevant images.
    - enable_entity_links (bool): Whether to generate hyperlinks for entities to external reference sources.
    
    Returns:
    - str: A prompt that instructs on generating an HTML formatted summary with advanced features.
    """
    prompt = (
    ...
    if include_images:
        prompt += (
            ...
            "3. Place the image prominently using: '<div style=\"text-align:center; margin:1.2em 0 1.8em 0;\"><img src=\"[IMAGE_URL]\" alt=\"[DESCRIPTIVE ALT TEXT]\" style=\"max-width:100%; border-radius:6px; box-shadow:0 2px 12px rgba(0,0,0,0.1);\"></div>'\n"
            "4. For the alt text, write a detailed description of the image content that relates to the article topic.\n"
    ...
    # Different entity formatting based on whether entity links are enabled
    if enable_entity_links:
        prompt += (
    ...
    prompt += (
        ...
        "       <li style=\"margin-bottom:0.8em; padding-left:1.5em; position:relative; font-size:0.95em;\"><span style=\"position:absolute; left:0; color:#aaa;\">○</span><strong style=\"color:#009E73\">Company XYZ</strong> set a record for quarterly <strong style=\"color:#666666\">profits</strong> in <strong style=\"color:#56B4E9\">Silicon Valley</strong>.</li>\n"
        "   </ul>\n"
        "   </div>\n"
        "   <hr style=\"height:2px; border:none; background: linear-gradient(to right, #ffffff, #e0e0e0, #ffffff); margin:1.5em 0;\">\n\n"        
        "LEGEND SECTION:\n"
