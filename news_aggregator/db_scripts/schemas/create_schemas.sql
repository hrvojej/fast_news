...
-- Event Timeline Table - with partitioning
...
-- Comments Table with partitioning
...
CREATE TABLE topics.topic_content (
   ...
   content_id TEXT NOT NULL, -- Changed to TEXT to accommodate comment_ids
   portal_id INT REFERENCES public.news_portals(portal_id) ON DELETE CASCADE,
...
BEGIN
    ...
    IF NEW.parent_topic_id IS NULL THEN
        ...
        NEW.level = 1;
    ELSE
        -- Handle child topics and prevent cycles
...
CREATE TABLE analysis.content_analysis (
    ...
    source_id TEXT NOT NULL, -- Changed to TEXT to accommodate comment_ids
    portal_id INT REFERENCES public.news_portals(portal_id) ON DELETE CASCADE,
    content_length INT,
    language_code VARCHAR(10),
...
-- 1. Fix for events.event_articles.article_id foreign key
-- First, create a function to validate article references across schemas
...
BEGIN
    -- Get the portal prefix for the given portal_id
    SELECT portal_prefix INTO portal_prefix
    FROM public.news_portals
    WHERE portal_id = NEW.portal_id;
...
-- 2. Fix for topics.topic_content content references
...
BEGIN
    -- Get portal prefix if portal_id is provided
    IF NEW.portal_id IS NOT NULL THEN
        SELECT portal_prefix INTO portal_prefix
        FROM public.news_portals
        WHERE portal_id = NEW.portal_id;
...
-- 3. Enhanced cycle prevention for topics.topics
...
BEGIN
    ...
    WITH RECURSIVE topic_tree AS (
        -- Base case: start with the parent topic
...
BEGIN
    ...
    IF NEW.relationship_type IN ('parent_of', 'child_of') THEN
        WITH RECURSIVE relationship_chain AS (
            ...
            WHERE NOT er.source_entity_id = ANY(rc.path)  -- Prevent infinite recursion
...
-- Create trigger for cycle prevention in entity relationships
...
-- Schedule a cron job to create partitions for timeline_entries
SELECT cron.schedule('0 0 1 * *', $$SELECT events.create_timeline_partitions(CURRENT_DATE, CURRENT_DATE + INTERVAL '6 months')$$);

-- Schedule a cron job to create partitions for comments
SELECT cron.schedule('0 0 1 * *', $$SELECT comments.create_comment_partitions(CURRENT_DATE, CURRENT_DATE + INTERVAL '6 months')$$);
