...
class PortalDataAdapter(DataAdapter):
    ...
    def update(self, portal_id: int, portal: Portal) -> Optional[Portal]:
        self.cursor.execute(
            ...
            """,
            (portal.portal_name, portal.portal_domain, portal.bucket_prefix, portal_id)
        )
        record = self.cursor.fetchone()
        if record:
            self.conn.commit()
...
```tool_code
<write_to_file>
<path>news_dagster-etl/news_aggregator/db_scripts/data_adapter.py</path>
<content>
import psycopg2
from dataclasses import dataclass
from typing import List, Optional
...
class EventArticleDataAdapter(DataAdapter):
    def get_by_id(self, event_id: int, article_id: int, portal_id: int) -> Optional[EventArticle]:
        self.cursor.execute(f"SELECT * FROM events.event_articles WHERE event_id = %s AND article_id = %s AND portal_id = %s", (event_id, article_id, portal_id))
        record = self.cursor.fetchone()
        if record:
            return EventArticle(*record)
        return None
